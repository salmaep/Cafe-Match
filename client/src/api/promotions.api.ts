...
export const promotionsApi = {
  ...
  getAvailability: (packageId: number, type: string) =>
    apiClient.get<SlotAvailability>(
      `/promotions/packages/${packageId}/availability`,
      {
        params: { type },
      },
    ),
  ...
  getMine: () => apiClient.get<Promotion[]>("/promotions/mine"),

  getById: (id: number) => apiClient.get<Promotion>(`/promotions/${id}`),
