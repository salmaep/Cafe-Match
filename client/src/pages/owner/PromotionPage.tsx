...
const TYPE_LABELS: Record<string, { name: string; description: string }> = {
  new_cafe: {
    ...
    description:
      "Highlighted map pin to announce your cafe presence. Distinct color/size from regular pins.",
  },
  featured_promo: {
    name: "Featured Promo",
    description:
      'Appear in the "Featured Cafes" section with custom title, description, and photo.',
  },
};
...
export default function PromotionPage() {
  ...
  useEffect(() => {
    Promise.all([promotionsApi.getPackages(), promotionsApi.getMine()])
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[promo.status] || ""}`}
                    >
