...
exports.seedPurposes = seedPurposes;
...
async function seedPurposes(dataSource) {
    const purposeRepo = dataSource.getRepository(purpose_entity_1.Purpose);
    const requirementRepo = dataSource.getRepository(purpose_requirement_entity_1.PurposeRequirement);
    const purposes = [
        {
            slug: 'me-time',
            name: 'Me Time',
    ...
    for (const data of purposes) {
        ...
        for (const req of requirements) {
            ...
            if (!existing) {
                await requirementRepo.save(requirementRepo.create({ ...req, purposeId: purpose.id }));
            }
        }
    }
    console.log('Purposes and requirements seeded successfully');
}
//# sourceMappingURL=purposes.seed.js.map