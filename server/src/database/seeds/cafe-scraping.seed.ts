...
const SCRAPING_DIR = path.resolve(
  __dirname,
  '../../../../scraping_cafe/output',
);
...
const OVERRIDES_JSON_PATH = path.join(
  SCRAPING_DIR,
  'cafe-purposes-overrides.json',
);
...
function parseHours(
  ...
  for (const h of hours) {
    ...
    const normalized = raw.replace(/\s*to\s*/i, '-').replace(/\./g, ':');
    out[key] = normalized;
  }
...
export async function seedScrapedCafes(dataSource: DataSource): Promise<void> {
  ...
  const cafesRaw: ScrapedCafe[] = JSON.parse(
    fs.readFileSync(CAFES_JSON_PATH, 'utf-8'),
  );
  console.log(`Loaded ${cafesRaw.length} scraped cafes`);

  // 2. Load purpose scores (optional — seed still works without it)
  const scoresMap = new Map<string, PurposeScore>();
  if (fs.existsSync(SCORES_JSON_PATH)) {
    const scoresList: PurposeScore[] = JSON.parse(
      fs.readFileSync(SCORES_JSON_PATH, 'utf-8'),
    );
  ...
  for (const c of cafesRaw) {
    ...
    try {
      ...
      const googleMapsUrl =
        c.url ||
        `https://maps.google.com/?q=${c.coordinates.lat},${c.coordinates.lng}`;
      ...
      if (scores) {
        for (const [rawPurpose, score] of Object.entries(
...
async function wipeOldCafeData(dataSource: DataSource): Promise<void> {
  ...
  for (const d of deletes) {
    try {
      const sql = d.where
        ? `DELETE FROM ${d.table} WHERE ${d.where}`
        : `DELETE FROM ${d.table}`;
...
async function assignPromotions(dataSource: DataSource): Promise<void> {
  ...
  if (!packages || packages.length === 0) {
    console.warn(
      '  No advertisement_packages found — skipping promo assignment',
    );
    return;
  }
  ...
  if (topCafes.length < 6) {
    console.warn(
      `  Only ${topCafes.length} top-rated cafes found — need at least 6`,
    );
    return;
  }
  ...
  const typeATemplates = [
    {
      ...
      highlightText:
        "Bandung's newest specialty coffee destination — freshly opened with a handpicked bean selection and panoramic views.",
